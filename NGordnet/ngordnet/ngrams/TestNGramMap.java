...
public class TestNGramMap {
    ...
    @Test
    public void testwordsHistoryVeryShort() {
...
// returns the count of the number of occurrences of fish per year between 1850 and 1933.
        TimeSeries fishCount=ngm.countHistory("airport");
        assertEquals(175702.0,fishCount.get(2007));
        assertEquals(173294.0,fishCount.get(2008));

        TimeSeries totalCounts=ngm.totalCountHistory();
        assertEquals(2563919231.0,totalCounts.get(1865));

// returns the relative weight of the word fish in each year between 1850 and 1933.
        TimeSeries airportWeight=ngm.weightHistory("airport",2007,2008);
        assertEquals(175702.0/28307904288.0,airportWeight.get(2007));

        TimeSeries requestCount=ngm.countHistory("request",2005,2008);
        assertEquals(697645.0,requestCount.get(2007));

        List<String> requestAndAirport=new ArrayList<>();
        requestAndAirport.add("request");
        requestAndAirport.add("airport");
        TimeSeries requestPlusAirportWeight=ngm.summedWeightHistory(requestAndAirport,2007,2008);

        double expectedRequesrPlusAirportWeight2007=(175702.0+697645.0)/28307904288.0;
        assertEquals(expectedRequesrPlusAirportWeight2007,requestPlusAirportWeight.get(2007));
    }
    @Test
    public void testCountHistoryVertShort() {
        ...
        TimeSeries<Number> request2005to2008 = ngm.countHistory("request");
        ...
        TimeSeries<Number> request2006to2007 = ngm.countHistory("request", 2006, 2007);
    ...
    @Test
    public void testWordsStartWith_q() {
    ...
    public void testOnLargeFile() {
        ...
        // returns the count of the number of occurrences of fish per year between 1850 and 1933.
        TimeSeries<Number> fishCount = ngm.countHistory("fish", 1850, 1933);
        assertEquals(136497.0, fishCount.get(1865), 1E-10);
        assertEquals(444924.0, fishCount.get(1922), 1E-10);

        TimeSeries<Number> totalCounts = ngm.totalCountHistory();
        assertEquals(2563919231.0, totalCounts.get(1865), 1E-10);

        // returns the relative weight of the word fish in each year between 1850 and 1933.
        TimeSeries<Number> fishWeight = ngm.weightHistory("fish", 1850, 1933);
        assertEquals(136497.0/2563919231.0, fishWeight.get(1865), 1E-7);

        TimeSeries<Number> dogCount = ngm.countHistory("dog", 1850, 1876);
        assertEquals(75819.0, dogCount.get(1865), 1E-10);

        List<String> fishAndDog = new ArrayList<>();
        fishAndDog.add("fish");
        fishAndDog.add("dog");
        TimeSeries<Number> fishPlusDogWeight = ngm.summedWeightHistory(fishAndDog, 1865, 1866);
